...
mkdir -p $(pwd)/../results_${OPTION}_8core
...
do
	...
	(./../${BINARY} -warmup_instructions ${N_WARM}000000 -simulation_instructions ${N_SIM}000000 -traces ${TRACE_DIR}/${TRACE} ${TRACE_DIR}/${TRACE} ${TRACE_DIR}/${TRACE} ${TRACE_DIR}/${TRACE} ${TRACE_DIR}/${TRACE} ${TRACE_DIR}/${TRACE} ${TRACE_DIR}/${TRACE} ${TRACE_DIR}/${TRACE}) > $(pwd)/../results_${OPTION}_8core/${name}_${N_WARM}M_${N_SIM}M.txt &
    else
	continue
    fi
done
