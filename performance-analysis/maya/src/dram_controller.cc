...
void MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue)
{
    ...
    for (uint32_t i=0; i<queue->SIZE; i++) {
           ...
           /*=================Changes for de-prioritizing the attacker starts================*/
        // if cpu is isolated, cpu is the attacker cpu, stall cycle of the cpu is > 0, don't schedule
	#if Deprioritize_attacker
           if (queue->entry[i].cpu == 0 && stall_the_attacker[queue->entry[i].cpu] > 0 )
           {
                   continue;
           }
	#endif
           /*=================Changes for de-prioritizing the attacker ends================*/
...
void MEMORY_CONTROLLER::process(PACKET_QUEUE *queue)
{
    ...
    if (bank_request[op_channel][op_rank][op_bank].cycle_available <= current_core_cycle[op_cpu]) {
		...
		int destination = get_slice_num((&queue->entry[request_index])->address);
...
int MEMORY_CONTROLLER::add_rq(PACKET *packet)
{
    ...
    uint32_t slice = get_slice_num(packet->address);
    if (all_warmup_complete < NUM_CPUS) {
        // if (packet->instruction) 
        //     upper_level_icache[packet->cpu]->return_data(packet);
        // else // data
        //     upper_level_dcache[packet->cpu]->return_data(packet);
        upper_level_slice[slice]->return_data(packet);    
        return -1;
    }
