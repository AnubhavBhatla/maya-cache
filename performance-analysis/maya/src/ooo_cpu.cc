...
uint64_t current_core_cycle[NUM_CPUS], stall_cycle[NUM_CPUS],rob_stall_count[NUM_CPUS], load_rob_stall_count[NUM_CPUS];
//Neelu: To get an idea of the contribution of loads in ROB head stalls.
uint64_t total_load_rob_stall_cycles[NUM_CPUS] = {0}, total_rob_stall_cycles[NUM_CPUS]= {0};
...
void O3_CPU::fetch_instruction()
{
  ...
  if((fetch_stall == 1) && (current_core_cycle[cpu] >= fetch_resume_cycle) && (fetch_resume_cycle != 0))
    ...
    for (uint32_t i=0; i<FETCH_WIDTH; i++) {
        ...
        /*
        printf("L1I ");
        if (fetch_packet.instr_id == 137)
            cout << "Fetch Address " << fetch_packet.address << endl;
        */
...
void O3_CPU::complete_instr_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb)
{
    ...
    if (is_it_tlb) {
        ...
        /*
        if (ROB.entry[rob_index].instr_id == 137)
            cout << "Instruction PA " << ROB.entry[rob_index].instruction_pa << endl;
        */
    }
    else
	...
	ITERATE_SET(i,queue->entry[index].rob_index_depend_on_me, ROB_SIZE) {
            ...
            if (is_it_tlb) {
                ...
                //if (ROB.entry[i].instr_id == 137)
                //    cout << "Instruction PA " << ROB.entry[i].instruction_pa << endl;
            }
            else
...
void O3_CPU::retire_rob()
{
    for (uint32_t n=0; n<RETIRE_WIDTH; n++) {
        ...
        if (ROB.entry[ROB.head].executed != COMPLETED ) {
	       //Neelu code for rob stalls

	    if(ROB.entry[ROB.head].stall_flag == 0 &&  warmup_complete[cpu] )
		{
	...
	if(ROB.entry[ROB.head].stall_flag)
	{
		...
		if(ROB.entry[ROB.head].load_stall_flag)
			total_load_rob_stall_cycles[cpu] += (current_core_cycle[cpu] - ROB.entry[ROB.head].stall_begin_cycle);
	}
...
{
//	 cout<<"Hi I am at OOO_CPU Total ROB stall cycles : \t"<<(total_rob_stall_cycles[cpu])<<endl;
}
