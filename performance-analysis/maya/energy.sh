...
#CEASER_8_core_4KB_attacker SCATTER_CACHE_8_core_4KB_attacker cEASER_8_core_4KB_attacker_with_New_ROB
		...
		if [ $is_divided_queues == 1 ]; then
				...
				if [ $CEASER_S_LLC == 0 ] || [ $CEASER_LLC == 1 ]; then
					file_name=$results_folder/${trace}-hashed_perceptron-no-no-no-ship-0-${partitions}-1-${CEASER_LATENCY}-${remap}-1-1core-1core_llcwith_MAYA.txt
				...
				file_name=$results_folder/${trace}-hashed_perceptron-no-no-no-ship-0-${partitions}-1-${CEASER_LATENCY}-${remap}-1-1core-1core_llcwith_MAYA.txt #_and_bfs.txt    #with_multi_step.txt        #with_multi_step.txt
				fi
				#	echo $file_name			
