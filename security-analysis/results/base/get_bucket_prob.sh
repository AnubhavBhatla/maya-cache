...
tot_exp=`ls raw_results/Base6.ExtraWays6.Exp* | wc -l`;
...
for (( exp_num=0; exp_num<$tot_exp; exp_num++ )) ; do
    ...
    line_num=`grep -n "Distribution of Bucket-Occupancy (Averaged across Ball Throws) => Used for P(Bucket = k balls) calculation" raw_results/Base6.ExtraWays6.Exp${exp_num}.out | awk -F':' '{print $1}'`
    start_line_num=$(($line_num+3))

    ## Get the Bucket-Occupancy for an experiment & add it to bucket_occ_temp counts
    tail -n+$start_line_num raw_results/Base6.ExtraWays6.Exp${exp_num}.out | head -n17 | awk -F':' '{print $2}' | awk '{print $1}' >bucket_occ_curr
...
echo "BallsInBucket-N Pr_obs(N)" > Base6.BucketProb.stat;
awk -v count=$total_ball_count '{print NR-1, $1/count}' bucket_occ_temp >> Base6.BucketProb.stat
column -t Base6.BucketProb.stat > temp
mv temp Base6.BucketProb.stat
